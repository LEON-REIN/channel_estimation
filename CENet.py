...
# model.summary()
# tf.keras.utils.plot_model(model, "./imgs/SIMO_CENet.png", show_shapes=True)
...
# model.load_weights("CENet/autograph/20210402-105120/ckpt/cp-0100.ckpt")

# Callback 1 -- tensorboard
# Execute "!tensorboard --logdir CENet\autograph" in Ipython
stamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")  # Time stamp
logdir = os.path.join('CENet', 'autograph', stamp)  # path to log
...
checkpoint_path = logdir + "\\ckpts\\cp-{epoch:04d}.ckpt"
...
model.fit(to_train,
          ...
          # validation_data=(data_to_test, labels_to_test),
          epochs=250,
...
# bb = aa.reshape(-1, 4)
# cc = np.argmax(bb, axis=1).reshape(-1, 64).astype(np.int)
